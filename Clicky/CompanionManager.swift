...
final class CompanionManager: ObservableObject {
    ...
    private lazy var claudeAPI: ClaudeAPI = {
        let api = ClaudeAPI(proxyURL: "\(Self.workerBaseURL)/chat", model: selectedModel)
        if let provider = AICLIProvider(rawValue: selectedAIProvider) {
            api.preferredProvider = provider
        }
        return api
    }()
    ...
    /// The AI CLI provider (Claude or Codex). Persisted to UserDefaults.
    @Published var selectedAIProvider: String = UserDefaults.standard.string(forKey: "selectedAIProvider") ?? "claude"

    func setSelectedAIProvider(_ provider: String) {
        selectedAIProvider = provider
        UserDefaults.standard.set(provider, forKey: "selectedAIProvider")
        if let aiProvider = AICLIProvider(rawValue: provider) {
            claudeAPI.preferredProvider = aiProvider
        }
    ...
    private func startOnboardingMusic() {
        ...
        do {
            ...
            onboardingMusicFadeTimer = Timer.scheduledTimer(withTimeInterval: 90.0, repeats: false) { [weak self] _ in
                let captured = self
                Task { @MainActor in
                    captured?.fadeOutOnboardingMusic()
                }
            }
        } catch {
    ...
    private func fadeOutOnboardingMusic() {
        ...
        onboardingMusicFadeTimer = Timer.scheduledTimer(withTimeInterval: stepInterval, repeats: true) { [weak self] timer in
            stepsRemaining -= 1
            unsafePlayer.volume -= volumeDecrement
            ...
            if stepsRemaining <= 0 {
                ...
                let captured = self
                Task { @MainActor in
                    captured?.onboardingMusicPlayer = nil
                    captured?.onboardingMusicFadeTimer = nil
                }
            }
        }
    }
    ...
    private func speakCreditsErrorFallback() {
        let utterance = AVSpeechUtterance(string: "Something went wrong. Please try again.")
    ...
    func setupOnboardingVideo() {
        ...
        onboardingDemoTimeObserver = player.addBoundaryTimeObserver(
            ...
            let captured = self
            Task { @MainActor in
                captured?.performOnboardingDemoInteraction()
            }
        }
