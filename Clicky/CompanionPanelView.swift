...
struct CompanionPanelView: View {
    ...
    private var aiProviderPickerRow: some View {
        HStack {
            ...
            HStack(spacing: 0) {
                aiProviderOptionButton(label: "Claude", providerID: "claude")
                aiProviderOptionButton(label: "Codex", providerID: "codex")
            }
            .modifier(GlassSegmentBackgroundModifier())
        }
        .padding(.vertical, 4)
    }

    private func aiProviderOptionButton(label: String, providerID: String) -> some View {
        let isSelected = companionManager.selectedAIProvider == providerID
        return Button(action: {
            companionManager.setSelectedAIProvider(providerID)
        }) {
            Text(label)
                .font(.system(size: 11, weight: .medium))
    ...
    private var modelPickerRow: some View {
        HStack {
            ...
            HStack(spacing: 0) {
                if companionManager.selectedAIProvider == "codex" {
                    modelOptionButton(label: "5.4", modelID: "gpt-5.4")
                    modelOptionButton(label: "5.4 mini", modelID: "gpt-5.4-mini")
                    modelOptionButton(label: "Codex", modelID: "gpt-5.3-codex")
                } else {
    ...
    private var dmFarzaButton: some View {
        Button(action: {
            ...
            .padding(.vertical, 10)
            .modifier(GlassCardBackgroundModifier())
        }
        .buttonStyle(.plain)
        .pointerCursor()
    }
...
/// Applies Liquid Glass as the panel background on macOS 26+,
/// falling back to the standard dark opaque background on older systems.
/// Wraps content in a GlassEffectContainer so inner glass effects
/// (model picker, DM card) are rendered as a unified glass surface.
...
/// Applies glass effect to segmented controls on macOS 26+,
/// falling back to a frosted material background on older systems.
private struct GlassSegmentBackgroundModifier: ViewModifier {
    func body(content: Content) -> some View {
        if #available(macOS 26, *) {
            content
...
/// Applies glass effect to card-like views on macOS 26+,
/// falling back to a frosted material background on older systems.
