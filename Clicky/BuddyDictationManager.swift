...
final class BuddyDictationManager: NSObject, ObservableObject {
    ...
    // Created fresh for each recording session. Reusing a single AVAudioEngine
    // causes outputFormat to return a stale sample rate after stop/restart (e.g.,
    // 48 kHz instead of the actual hardware 24 kHz), leading to format mismatch
    // crashes: "Input HW format and tap format not matching".
